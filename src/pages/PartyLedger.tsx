...
export default function PartyLedger() {
  ...
  if (!party) return <div className="p-8 text-center text-brand-blue/30 uppercase tracking-widest font-black text-xs">Accessing Secure Vault...</div>;
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-32">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="text-center">
            <h1 className="text-3xl font-black text-brand-blue dark:text-[#F7F7F7] tracking-tighter">{party.name}</h1>
            <p className="text-[10px] font-black text-brand-blue/30 dark:text-[#A0A0A0] uppercase tracking-widest mt-1">
      ...
      <div className="bg-white dark:bg-[#111111] p-8 rounded-[40px] border border-brand-blue/5 dark:border-[#222222] shadow-xl text-center relative overflow-hidden group">
        ...
        <p className="text-[12px] font-black text-brand-blue/30 dark:text-[#A0A0A0] uppercase tracking-[0.2em] mb-2">Net Financial Standing</p>
        <h2 className={`text-6xl font-black tracking-tighter mb-4 transition-colors ${netBalance >= 0 ? 'text-brand-green' : 'text-brand-red'}`}>
            ₹{Math.abs(netBalance).toLocaleString('en-IN')}
        </h2>
        <div className={`inline-flex items-center gap-2 px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-widest ${netBalance >= 0 ? 'bg-brand-green/10 text-brand-green' : 'bg-brand-red/10 text-brand-red'}`}>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
            ...
            <span className="text-[12px] font-black uppercase tracking-widest">Cash Out (You Gave)</span>
        </button>
        <button 
        ...
        >
            ...
            <span className="text-[12px] font-black uppercase tracking-widest">Cash In (You Got)</span>
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-[#111111] rounded-[32px] border border-brand-blue/5 dark:border-[#222222] shadow-sm overflow-hidden">
        <div className="p-6 border-b border-brand-blue/5 flex items-center justify-between">
            <h3 className="text-xs font-black text-brand-blue/40 uppercase tracking-widest flex items-center gap-2">
            ...
            <table className="w-full text-left">
                <thead>
                    <tr className="bg-brand-blue/5 dark:bg-brand-blue/10">
                        <th className="px-6 py-4 text-[10px] font-black text-brand-blue/40 uppercase tracking-widest">Entry Detail</th>
                        <th className="px-6 py-4 text-[10px] font-black text-brand-blue/40 uppercase tracking-widest text-right">Cash Out</th>
                        <th className="px-6 py-4 text-[10px] font-black text-brand-blue/40 uppercase tracking-widest text-right">Cash In</th>
                        <th className="px-6 py-4 text-[10px] font-black text-brand-blue/40 uppercase tracking-widest text-right">Balance</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-brand-blue/5">
                            ...
                            <tr key={tx.id} className="hover:bg-brand-blue/5 transition-colors group">
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-[10px] font-black text-brand-blue/30 dark:text-[#A0A0A0] uppercase tracking-tighter mb-0.5">{format(tx.dateTime, 'MMM dd, yyyy')}</p>
                                            <p className="font-bold text-brand-blue dark:text-white line-clamp-1">{tx.remarks || "No remarks filed"}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-5 text-right font-black text-brand-red">
                                    {tx.type === 'CASH_OUT' ? `₹${tx.amount.toLocaleString('en-IN')}` : '-'}
                                </td>
                                <td className="px-6 py-5 text-right font-black text-brand-green">
                                    {tx.type === 'CASH_IN' ? `₹${tx.amount.toLocaleString('en-IN')}` : '-'}
                                </td>
                                <td className="px-6 py-5 text-right">
                                    <div className="flex flex-col items-end">
                                        <p className={`font-black tracking-tighter ${balance >= 0 ? 'text-brand-green' : 'text-brand-red'}`}>
                    ...
                    {transactions.length === 0 && (
                        <tr>
                            <td colSpan={4} className="py-20 text-center text-brand-blue/20 font-black uppercase tracking-[0.3em] text-[10px]">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#111111] rounded-[40px] w-full max-w-sm border border-brand-blue/10 p-10 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h2 className="text-2xl font-black text-brand-blue dark:text-[#F7F7F7] tracking-tighter">
                        Log {txType === 'CASH_OUT' ? 'Cash Out' : 'Cash In'}
                    </h2>
                    <p className="text-[10px] font-black text-brand-blue/30 uppercase tracking-widest mt-0.5">Recording Ledger entry</p>
                </div>
            </div>

            <div className="space-y-5">
              <div>
                <label className="block text-[10px] font-black text-brand-blue/40 uppercase tracking-widest mb-1.5 ml-1">Recording Date</label>
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full pl-12 pr-4 py-3.5 bg-neutral-50 dark:bg-[#1A1A1A] border border-brand-blue/5 rounded-2xl outline-none font-black text-brand-blue dark:text-white"
                    />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-black text-brand-blue/40 uppercase tracking-widest mb-1.5 ml-1">Amount</label>
                <div className="relative">
                    ...
                    <input 
                        ...
                        placeholder="0.00"
                        autoFocus
                        className="w-full pl-10 pr-4 py-3.5 bg-neutral-50 dark:bg-[#1A1A1A] border border-brand-blue/5 rounded-2xl outline-none font-black text-brand-blue dark:text-white text-xl"
                    />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-black text-brand-blue/40 uppercase tracking-widest mb-1.5 ml-1">Notes / Remarks</label>
                <div className="relative">
                    ...
                    <textarea 
                        ...
                        className="w-full pl-12 pr-4 py-3.5 bg-neutral-50 dark:bg-[#1A1A1A] border border-brand-blue/5 rounded-2xl outline-none font-black text-brand-blue dark:text-white h-24 resize-none placeholder:text-brand-blue/10"
                    />
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-4">
                <button 
                    ...
                    className="flex-1 py-4 text-[10px] font-black uppercase tracking-widest text-brand-blue/40 hover:bg-brand-blue/5 rounded-2xl transition-all"
                >
                    Cancel
                </button>
                <button 
                    onClick={handleAddTransaction} 
                    className={`flex-1 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg transition-all active:scale-95 ${txType === 'CASH_OUT' ? 'bg-brand-red shadow-brand-red/20' : 'bg-brand-green shadow-brand-green/20'} text-white`}
                >
                    Log Entry
                </button>
              </div>
