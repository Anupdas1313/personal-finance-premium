...
import { Wallet, AlertCircle, Mail, Sparkles } from 'lucide-react';
import { useToast } from '../context/ToastContext';

export default function Auth() {
  const { user, signIn, signUp, signInWithGoogle, resetPassword, signInAsDemo } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#060608] flex flex-col justify-center px-5 py-6 sm:px-6 lg:px-8 relative overflow-hidden antialiased">
      ...
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-brand-green/5 blur-[100px] pointer-events-none" />
          ...
          {isVerificationSent ? (
            <div className="text-center py-4">
              ...
              <button
                ...
                className="w-full bg-brand-green hover:bg-brand-green/90 text-white font-semibold h-[52px] rounded-xl transition-all shadow-md text-[15px]"
              >
                Log In
              </button>
            </div>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex justify-between items-center mb-1.5 ml-1">
                    ...
                    {isLogin && (
                      <button 
                        ...
                        className="text-[13px] font-semibold text-brand-green hover:text-brand-green/80 transition-colors"
                      >
                        Forgot?
                      </button>
                    )}
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-brand-green hover:bg-brand-green/90 text-white font-semibold h-[52px] rounded-xl transition-all shadow-md disabled:opacity-50 text-[15px] active:scale-[0.98]"
                  >
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={loading}
                  className="flex-1 bg-white dark:bg-[#15151A] hover:bg-neutral-50 dark:hover:bg-[#1A1A1E] border border-neutral-200 dark:border-[#1A1A1E] text-neutral-700 dark:text-white font-semibold h-[52px] rounded-xl transition-all flex items-center justify-center gap-3 disabled:opacity-50 shadow-sm text-[15px] active:scale-[0.98]"
                >
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex-1 bg-gradient-to-br from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/20 text-amber-600 dark:text-amber-400 font-semibold h-[52px] rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-sm text-[15px] active:scale-[0.98]"
                >
              ...
              <div className="mt-8 text-center">
                ...
                >
                  {isLogin ? (
                    ...
                    <>Already have an account? <span className="text-brand-green font-semibold">Sign in</span></>
                  )}
                </button>
              </div>
            </>
